    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://203.170.69.170:8070/api/INVAPI/MRNList?type=C`,
          {
            headers: {
        ...
        const MRNListCivil = await response.data;

        setData(MRNListCivil);
        setIsLoading(false);
      } catch (error) {
        console.error(error);
      }
