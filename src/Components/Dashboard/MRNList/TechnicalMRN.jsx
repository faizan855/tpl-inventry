    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://203.170.69.170:8070/api/INVAPI/MRNList?type=T`,
          {
            headers: {
        ...
        const MRNListTechnical = await response.data;

        setData(MRNListTechnical);
        setIsLoading(false);
      } catch (error) {
        console.error(error);
      }
