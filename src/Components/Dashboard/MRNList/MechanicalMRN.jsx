    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://203.170.69.170:8070/api/INVAPI/MRNList?type=M`,
          {
            headers: {
        ...
        const MRNListMechanical = await response.data;

        setData(MRNListMechanical);
        setIsLoading(false);
      } catch (error) {
        console.error(error);
      }
